...
def itzuli(snomed,ema,path,engHash,spaHash,kode_en,kode_es,tok_kop,adj_hiz,kat_hiz):
    ...
    for el in kode_es:#kode_es:
        ...
        if ordList:
            ...
            egu_lag = {'konTBX':konId,'ordList':1,'term':terminoS.getId(),'ema':ema,'zb':zb}
            #print(egu_lag)
    ...
    for el in kode_en:#konTBX.getTerminoak('en'):
        ...
        if ordList:
            ...
            #konTBX.eguneratu(ordList,terminoS.term,ema,zb)
            egu_lag = {'konTBX':konId,'ordList':0,'term':terminoS.getId(),'ema':ema,'zb':zb}
        ...
        if not itzulia:
            if tok_kop == 1:
                ...
                if '+?' not in irteera[0]:
                    ...
                    #konTBX.eguneratu(ordList,terminoS.term,ema,zb)
                    egu_lag = {'konTBX':konId,'ordList':ord_key,'term':terminoS.getId(),'ema':ema,'zb':zb}
                    #print(egu_lag)
                    eguneratzekoak.append(egu_lag)
                    ema.setTerminoa(term,'en','bai')
                    itzulia = True
